...
export const ChatContainer = () => {
    ...
    const getGPTResponse = async (text: string) => {
        ...
        await fetchGPTResponse(text, context).then(response => {
            if (response.status === 'success') { 
                const gptResponse = response.data?.response as string;
                const context = response.data?.context as number[]
                const message = {
                    "user": "Chat-GPT",
                    "picture": "https://static.vecteezy.com/system/resources/thumbnails/021/059/825/small_2x/chatgpt-logo-chat-gpt-icon-on-green-background-free-vector.jpg",
                    "text": gptResponse
                }
    ...
    return(
        <div className="flex flex-col min-h-screen overflow-y-auto hidden-scrollbar items-center relative">
            ...
            <MessageForm onChange={onChange} addMessage={addUserMessages} textInput={textInput}/>
            </div>
        </div>
    )
}